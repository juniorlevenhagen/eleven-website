...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    console.log("Verificando variáveis de ambiente:", {
      ...
      tableName: process.env.AIRTABLE_CONTACT,
    });

    if (
      !process.env.AIRTABLE_API_KEY ||
      !process.env.AIRTABLE_BASE_ID ||
      !process.env.AIRTABLE_CONTACT
    ) {
      console.error("Variáveis de ambiente faltando:", {
        apiKey: !process.env.AIRTABLE_API_KEY,
        baseId: !process.env.AIRTABLE_BASE_ID,
        table: !process.env.AIRTABLE_CONTACT,
      });
    ...
    const record = {
      fields: {
        fldA7HIgnUgE9qYhY: name,
        fld3ClUThq5DetGQv: email,
        fld3DoEcmIgiHqkuU: message,
      },
    };
    ...
    const res = await fetch(
      `https://api.airtable.com/v0/${process.env.AIRTABLE_BASE_ID}/${process.env.AIRTABLE_CONTACT}`,
      {
        method: "POST",
        headers: {
