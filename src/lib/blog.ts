...
function mapRecordToPost(record: AirtableRecord): BlogPost | null {
  ...
  if (!readTime) {
    const readTimeNum = getField(fields, "Estimated Read Time (min)", "estimated_read_time");
  ...
  let coverImage = getStringField(fields, "Cover Image", "cover_image", "CoverImage");
  if (!coverImage) {
    const attachments = getField(fields, "Featured Image", "featured_image");
